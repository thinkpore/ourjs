...
var WebSvr = module.exports = function(options) {
  ...
  */
  var SessionParser = function() {
    ...
    //session id
    self.sid  = null; //initialise sid and val
    self.val  = null;
  };
  ...
  SessionParser.prototype = {
    ...
    */
    , newID: function(appendLen) {  //generate session id. return new sessin id.
    ...
    , create: function() {
      var self = this;
      self.sid  = self.newID(); //new session id
      self.val  = { __lastAccessTime: +new Date() };
      return self;
    }
  ...
  };  //end SessionParser.prototype


  var errorRender = function(err, customErrorMsg, outFn) {
    var errorMsg
      = (customErrorMsg || '')
      + '\n'
    ...
    if (outFn) {
      Settings.showError
        ? outFn('<pre>' + errorMsg.replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</pre>')
        : outFn();
    }
  }
  ...
  var Parser = function(req, res, mapper) {
    ...
    var handle = function() {
      try {
        ...
        errorRender(err, 'Error ' + new Date().toISOString() + ' ' + req.url, res.end)
      }
    }; //end handle
    ...
    var parseSession = function() {
      ...
      if (mapper.session && typeof req.session == "undefined") {
        req.session = new SessionParser(); //The type of req.session is SessionParser.
    ...
    */
    var parseCookies = function() {
      var cookie  = req.headers.cookie
		...
		//cookie format from client sent.
		//key=value;key=value;...
      if (cookie) {
        ...
        for (var i = 0; i < cookieArr.length; i++) {
          var strCookie = cookieArr[i]
            ...
            ;
		//key:strCookie.substr(0, idx).trim()
		//value:strCookie.substr(idx + 1).trim())
  ...
  */
  var MemoryStore = (function() {
    ...
    var list; //it's an object.

    //force update session in list, convert to big int
    //get session in list, if undefined create new one
    var get = function(sid, cb) {
      !list && init(); //if list is empty, then call init() function.
      !list[sid] && (list[sid] = {}); //if list[sid] don't existed, then set list[sid] to {}.
      cb && cb(list[sid]); //if cb is not empty, then call cb function.
    };
    ...
    */
	...
	//initialise list variable, and delete expired session at some interval.
    var init = function() {
      list = {};
  ...
  var FileStore = (function() {
    ...
    var get = function(sid, cb) {
      ...
      fs.readFile(getPath(sid), function(err, data) {
		...
		//get session from the file.seesion be saved as data.
        try {
          session = JSON.parse(data);
        } catch (e) {
          Logger.debug(e);
        }
		//if catch exception, cann't call cb function.This is a defect.
        cb && cb(session); //when pasrse session success, call callbackfunction. 
      });
    };
  ...
  Mapper.prototype = {
    ...
    */
    matchString: function(req, isHandler, expression) {
      ...
      if (expression.indexOf('/:') < 0) { //if doesn't ';:' substring
        var idx = reqUrl.indexOf(expression); //match expression
    ...
    */
    parseUrl: function(expression, reqUrl) {
      ...
      var parts   = expression.split('/')    //return string array.
        , start   = expression.charAt(0) === '/' ? 0 : 1  //if first charactor is '/', then skip.
        , urls    = reqUrl.split('/')
        , params  = {}
        ;
  ...
  */
  var Template = (function() {
    ...
    var getFile = function(filename, cb) {
      ...
      if (Settings.templateCache && cachedTemplate) { //if enable template, and the cache that filename corresponding  exist.
    ...
    return {
        //render templates
        render: function(tmplUrl, _model, outFn, isRawTmpl) {
          var res = this
            , end = outFn || res.end
            ...
            try {
              ...
              */
              var errStack  = err.stack || ''
                , lineStr   = '<anonymous>:'
                , idx       = errStack.indexOf(lineStr)
                , errmsg    = res.req.url + '@' + tmplUrl.substr(0, 400)
              ...
              errorRender(err, errmsg, end)
            }
          };

          isRawTmpl
            ? getInclude(tmplUrl, render)
            : getFile(tmplUrl, render)
        }
      , renderRaw: function(rawTmpl, model, outFn) {
          Template.render(rawTmpl, model, outFn, true)
        }
  ...
  var fileHandler = function(req, res) {
	...
	//if has query string, then discard query string.
    ...
    var handlePath = function(phyPath) {
      fs.stat(phyPath, function(err, stat) {
        ...
        if (stat.isFile()) {
		  ...
		  //? The file is not modified.so client can load this file from local cache.
          if (Settings.cache && stat.mtime <= cacheTime) {
            res.writeHead(304);
            res.end();

          // Else send "not modifed"
		  //? The file is modified.so server send last file to client.
          } else {
  ...
  var requestHandler = function(req, res) {
    ...
    res.end = function() {
	  ...
	  //http.ServerResponse.headersSent:boolean
      if (!res.headersSent) {
